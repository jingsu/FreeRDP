...
#endif
#define RDPVECTERR(fmt, ...) DEBUG_PRINT("ERR_RDPVECT" " %s (%d): ", fmt, ## __VA_ARGS__)
...
_hexdump(void* buf, int len)
{
    ...
    while( t < len )
    {
        fprintf(stderr, "%03d : ", t);
        for( c = 0; c < maxcol && t + c < len; c++ )
        {
            fprintf(stderr, "%02x ", p[c] & 0xff);
        }
        for( c = 0; c < maxcol && t + c < len; c++ )
        {
            if( isalnum(p[c]) )
